...
def main(
    ...
    if run_training:
        ...
        mlflow.set_experiment("aziz_sancar_ft")

        with mlflow.start_run(run_name=pipeline_args["run_name"]):
            mlflow.log_param("pipeline", "training")
            mlflow.log_param("config_path", str(pipeline_args["config_path"]))
            mlflow.log_param("timestamp", dt.now().isoformat())

            try:
                # ZenML pipeline'ı çalıştır
            ...
            # Örnek metrik loglama
            mlflow.log_metric("train_loss", 0.045)
            mlflow.log_metric("val_accuracy", 0.92)

            logger.info("✅ Training pipeline logged successfully to MLflow.")
